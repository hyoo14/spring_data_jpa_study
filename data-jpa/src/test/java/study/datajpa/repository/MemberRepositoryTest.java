...
//        findMember1.setUsername("member!!!!!!!!!!!!"); //변경감지(dirty checking) <-요걸로 변경하는 것이 정석.
    ...
    public void returnType() {
        ...
        //jpa는 결과 없으면 noresultexception 터트림. 하지만 data jpa에서는 exception안 터트림
        //optional 쓰면 걍 다 괜찬. 없으면 그냥 엠티옵셔널..
        //근데 2개면 예외터짐 NonUniqueResultException
    ...
    public void paging(){
        ...
        Page<MemberDto> toMap = page.map(m -> new MemberDto(m.getId(), m.getUsername(), null)); //page유지하면서 dto로 바꿀 수 있음

        //페이지 계산 공식 적용.. totalPage = totalCount/size ... 마지막페이지.. 최초 페이지...
    ...
    @Test
    public void bulkUpdate(){ //벌크 때려버리면 영속성 컨텍스트 반영 안 된 것들이 문제가 생길 수 있음
        //given
        memberRepository.save(new Member("member1", 10));
...
//        em.flush(); //벌크 이후에는 영속성 컨텍스트 날려야함!
//        em.clear();
        //jpql적으면 쿼리먼저 날리고 그 다음에 실행됨!

        List<Member> reslut = memberRepository.findByUsername("member5");
        Member member5 = reslut.get(0);
    ...
    public void queryHint(){
        ...
        findMember.setUsername("member2"); //변경감지하여 db에 업뎃쿼리나감(dirty checking) 엿지만 hint를 통해 무시
        //성능 최적화를 위해 요정도 씀.. 많이는 안 씀
        //성능 테스트 해보고 넣으시오
